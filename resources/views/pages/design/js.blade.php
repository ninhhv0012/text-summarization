<script>
    ...
    var dt = $("#kt_customers_table").DataTable({
        ...
        ordering: false,
        ajax: {
            url: "{{ route('designs.show', '') }}" + "/get-datatable",
            data: function(d) {
        ...
        columnDefs: [{
...

            {
                ...
                render: function(data, type, row) {
                    ...
                    return ' <a download="' + data + '" href="{{ route('image', '') }}/' +
                        data +
                        '">Link tải</a> ';
                },
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    return ' <a href="{{ route('image', '') }}/' + data +
                        '" target="_blank"><img src="{{ route('image', '') }}/' + data +
                        '" width ="50px" > </a> ';
                },
            },
            {
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    ...
                    switch(data){
                        case "0":
                            return '<span id="btn-active" data-id="' + row.id + '" data-activation="' + row.status + '"  class="badge badge-primary">New</span>';
                            break;
                        case "1":
                            return '<span id="btn-active" data-id="' + row.id + '" data-activation="' + row.status + '"  class="badge badge-warning">Processing</span>';
                            break;
                        case "2": 
                            return '<span id="btn-active" data-id="' + row.id + '" data-activation="' + row.status + '"  class="badge badge-success">Finish</span>';
                            break;
                        case "3":
                            return '<span id="btn-active" data-id="' + row.id + '" data-activation="' + row.status + '"  class="badge badge-danger">Redesign</span>';
                            break;
                        case "4":
                            return '<span id="btn-active" data-id="' + row.id + '" data-activation="' + row.status + '"  class="badge badge-info">Complete</span>';
                            break;
                        default:
                            return '<span id="btn-active" data-id="' + row.id + '" data-activation="' + row.status + '"  class="badge badge-danger">Error</span>';
                            break;
                    }
            ...
            },
            {
                ...
                render: function(data, type, row) {
                    if (data == null) {
                        return '<span class="btn btn-primary" name="btn-take" data-id="' + row.id + '">Take</span>';
                    } else {
                        return data;
                    }
                }
    ...
    $("#add_modal").on('click', function() {
        ...
        $('#kt_modal_add_customer_form #text_img').text('Choose image');
        $('#blah').attr("src", "");
        $('#kt_modal_add_customer_form h2').text('Add design');
